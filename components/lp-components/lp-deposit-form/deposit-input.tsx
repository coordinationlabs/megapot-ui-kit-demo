...
export function DepositInput({
    ...
    const formattedBalance = walletBalance !== undefined
        ? parseFloat(formatUnits(walletBalance, tokenDecimals)).toLocaleString(undefined, { maximumFractionDigits: 4 }) // Show more precision
        : '0';
    ...
    return (
        <div className="mb-4">
            ...
            <input
                ...
                placeholder="0.0"
                className={`mt-1 block w-full h-10 rounded-md border-2 ${parseError ? 'border-red-500' : 'border-gray-300'} shadow-sm focus:border-emerald-500 focus:ring focus:ring-emerald-500 focus:ring-opacity-50 sm:text-sm px-3`}
                value={depositAmountStr}
                onChange={(e) => setDepositAmountStr(e.target.value.replace(/[^0-9.]/g, ''))} // Basic input filtering
